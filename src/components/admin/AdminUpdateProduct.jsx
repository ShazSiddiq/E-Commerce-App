...
export default function AdminUpdateProduct() {
    ...
    let { update, getSingle } = useContext(Product)
    ...
    async function getAPIData() {
        ...
        var preresponse = await getSingle(item)
        if (preresponse.result === "done") {
            setProduct(preresponse.data)
        }
        else
            alert(preresponse.message)

        var response = await getMainCategory()
        var items=response.data.filter((item)=>item.name!==preresponse.data.maincategory)
        if (response.result === "done") {
        ...
        var items=response.data.filter((item)=>item.name!==preresponse.data.subcategory)
        if (response.result === "done") {
        ...
        var items=response.data.filter((item)=>item.name!==preresponse.data.brand)
        if (response.result === "done") {
            setBrand(items)
        }
        else
                ...
                <div className="col-xl-10 col-lg-9 col-md-8 col-sm-6 col-12">
                    ...
                    <form onSubmit={postData}>
                        <div className="mb-3">
                            ...
                            <input type="text" className="form-control" name='name' placeholder='Enter Product Name' value={product.name}  onChange={getData} />
                        </div>
                        ...
                        <div className="row mb-3">
                            <div className="col-md-6 col-12">
                                ...
                                <input type="text" className="form-control" name='color' placeholder='Enter Product Color'  onChange={getData} value={product.color} />
                            </div>
                            <div className="col-md-6 col-12">
                                <label className="form-label">Size</label>
                                <input type="text" className="form-control" name='size' placeholder='Enter Product Size'  onChange={getData} value={product.size} />
                            </div>
                        </div>
                        <div className="row mb-3">
                            <div className="col-md-6 col-12">
                                <label className="form-label">Base Price</label>
                                <input type="number" className="form-control" name='baseprice' placeholder='Enter Product Baese Price'  onChange={getData} value={product.baseprice} />
                            </div>
                            <div className="col-md-6 col-12">
                                <label className="form-label">Discount</label>
                                <input type="number" className="form-control" name='discount' placeholder='Enter Product Discount'  onChange={getData} min={0} value={product.discount} />
                            </div>
                        </div>
                        <div className="mb-3">
